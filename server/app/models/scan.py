...
class Scan(db.Model):
    ...
    def get_latest_result(self):
        ...
        if not self.results:
            return None
        # Filter out results with no start_time, just in case
        results_with_time = [r for r in self.results if getattr(r, "start_time", None)]
        if not results_with_time:
            return None
        # Sort by start_time descending and return the first
        print(
            "Latest result:",
            sorted(results_with_time, key=lambda r: r.start_time, reverse=True)[0],
        )
        return sorted(results_with_time, key=lambda r: r.start_time, reverse=True)[0]
    ...
    def to_dict(self):
        ...
        return {
            ...
            "get_latest_result": (latest_result.to_dict() if latest_result else None),
        }
